...
export class VisualizarTarefasComponent implements OnInit {
  ...
  excluirTarefa(id: number) {
    ...
    }).catch(error => {console.log(error); Swal.fire('Erro!', 'A tarefa não foi deletada.', 'error');});
  }
    ...
    if (this.form.valid) {
      ...
      this.tarefaService.atualizarTarefa(this.form.value.id, editarTarefa).then(resposta =>{
        ...
        if(resposta === 1){
          ...
          /*Nenhuma tarefa foi encontrada com o id passado, ou , nenhuma alteração foi necessária*/
